...
<div>
    <div style="margin-bottom: 20px">
        ...
        <el-date-picker v-model="queryInfo.createStartTime" type="date" placeholder="创建日期起" value-format="yyyy-MM-dd" class="input-style">
        </el-date-picker>
        <el-date-picker v-model="queryInfo.createEndTime" type="date" value-format="yyyy-MM-dd" placeholder="创建日期止" class="input-style">
        </el-date-picker>
    </div>
    <div style="margin-bottom: 20px">
    ...
    <el-table :data="tableData" border :max-height="tableMaxHeight" :header-cell-style="{'text-align': 'center'}" :cell-style="{'text-align': 'center'}" style="width: 90%">
        <el-table-column fixed label="序号" width="60">
            <template slot-scope="scope">
                {{ scope.$index*queryInfo.pageNum + 1 }}
            </template>
        </el-table-column>
        <el-table-column prop="areaId" label="区域平台" width="180">
        </el-table-column>
        <el-table-column prop="costType" label="费用类别" width="120">
        </el-table-column>
        <el-table-column prop="receivableId" label="应收结算方" width="180">
        </el-table-column>
        <el-table-column prop="payableId" label="应付结算方" width="180">
        </el-table-column>
        <el-table-column prop="remark" label="说明" width="250">
        </el-table-column>
    ...
    <div style="width: 90%;display: flex;justify-content: flex-end;padding-top: 15px">
        <el-pagination align='center' @size-change="handleSizeChange" @current-change="handleCurrentChange" background :current-page="queryInfo.pageNum" :page-sizes="[10,20,50,100,500]" :page-size="queryInfo.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
        </el-pagination>
    </div>
    <!--新增弹窗-->
    <el-dialog :title="titleText" :visible.sync="addShow" width="50%">
        <div class="add-dialog">
            ...
            <div class="line-item">
                <p>说明</p>
                <el-input type="textarea" placeholder="请输入内容" style="flex: 1" maxlength="50" v-model="addList.remark" :disabled="isSee" show-word-limit>
                </el-input>
            </div>
            <template v-if="isSee">
                <h5>历史记录</h5>
                <el-table :data="historicalRecords" border size="mini" :header-cell-style="headerStyle" :cell-style="{'text-align': 'center'}" style="width: 85%">
                    <el-table-column label="区域平台" width="110">
                        <template slot-scope="scope">
                    ...
                    <el-table-column label="应收结算方" width="110">
                        <template slot-scope="scope">
                            {{ scope.row.object.receivableId }}
                        </template>
                    </el-table-column>
                    <el-table-column label="应付结算方" width="110">
                        <template slot-scope="scope">
