...
<div class="user">
    ...
    <el-form :inline="true" class="form-inline">
        <el-form-item label="">
            <el-input placeholder="==用户类型==" v-model="queryObj.name"></el-input>
        </el-form-item>
 ...
 <!--批量操作-->
    ...
    <vue-add ref="addBox" title="添加用户类型" :addObj="addObj" :getList="getList" :postData="api.postData" v-slot="scope">
        <vee-item rules="required|unique:/main/userRole/data-unique" v-slot="{ failedRules  }">
            <el-form-item label="用户类型">
                <el-input placeholder="==用户类型==" v-model="scope.addObj.name" maxlength="32">></el-input>
         ...
         <vee-item rules="required|integer|unique:/main/userRole/data-unique-vid" v-slot="{ failedRules  }">
            <el-form-item label="标记id">
                <el-input placeholder="==标记id==" v-model="scope.addObj.vid" maxlength="8">></el-input>
                <span class="text-danger" v-if="failedRules.required">标记id不能为空！</span>
                <span class="text-danger" v-if="failedRules.integer">必须为整型数字！</span>
                <span class="text-danger" v-if="failedRules.unique">标记id已存在！</span>
            </el-form-item>
        </vee-item>
    ...
    <vue-edit ref="editBox" title="修改用户类型" :editObj="editObj" :getList="getList" :putData="api.putData" v-slot="scope">
        ...
        <vee-item :rules="'required|unique:/main/userRole/data-unique,'+scope.editObj._id" v-slot="{ failedRules  }">
         ...
         <vee-item :rules="'required|integer|unique:/main/userRole/data-unique-vid,'+scope.editObj._id" v-slot="{ failedRules  }" v-if="getUserVid===2">
            <el-form-item label="标记id">
                <el-input placeholder="==标记id==" v-model="scope.editObj.vid" maxlength="8">></el-input>
                <span class="text-danger" v-if="failedRules.required">标记id不能为空！</span>
                <span class="text-danger" v-if="failedRules.integer">必须为整型数字！</span>
                <span class="text-danger" v-if="failedRules.unique">v标记id已存在！</span>
            </el-form-item>
        </vee-item>
    ...
    <vue-dtl ref="dtlBox" title="查看详情" :dtlObjs="dtlObjs" v-slot="scope">
        <el-descriptions class="margin-top" title="" :column="2" border size="small">
            ...
            <el-descriptions-item label="标记id" label-class-name="table-1-5" content-class-name="table-3-5" >{{scope.dtlObj.vid}} </el-descriptions-item>
            <el-descriptions-item label="用户类型" label-class-name="table-1-5" content-class-name="table-3-5"> {{scope.dtlObj.name}}</el-descriptions-item>
...
export default {
    ...
    data() {
        return {
            ...
            // axios 接口
            api: {
                getData: mianApi.userRole.getData,
                getDataDtl: mianApi.userRole.getDataDtl,
                postData: mianApi.userRole.postData,
                putData: mianApi.userRole.putData,
                deleteData: mianApi.userRole.deleteData,
